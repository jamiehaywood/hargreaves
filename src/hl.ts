...
import { getVT } from "./authentication/getVT";
import { postUsernameDob } from "./authentication/postUsernameDob";
import { getRequiredSecureNumbers } from "./authentication/getSecureNumbers";
...
export class HL {
  ...
  async authenticate(
    ...
    const hl_vt = await getVT();

    await postUsernameDob(hl_vt, username, dateOfBirth);

    const secureNumbers = await getRequiredSecureNumbers();
    ...
    for (const account of accounts) {
      accountsCollection[
        $(account)
          .text()
          .trim()
      ] = await Account.getAccount($(account).attr("href"));
    }
...
class Account implements IAccount {
  ...
  public static getAccount = async (url?: string) => {
    ...
    let $ = cheerio.load(data);
    account.stockValue = $("#stock_total_header")
      .text()
      .trim();
    account.cashValue = $("#cash_total_header")
      .text()
      .trim();
    account.totalValue = $("#account_total_header")
      .text()
      .trim();
    account.totalGain = $("#gaingbp_total")
      .text()
      .trim();
